...
- `extra_exists.py`: to check whether an optional library is needed for the compilation or not.
...
Optional libraries like `parhip`, `parmetis`, `caliper`, or `liwik` have their files removed from the source file list by checks on the `OPT_<LIBRARY>` variables.
