      ...
      do j = 1, nnb
        ...
        if (.not. is_boundary) then
          ...
          type is (upwind_field)
            call calc_advection_coeff(phi, sgn * mf(index_f), 0, adv_coeff)
          type is (gamma_field)
            call calc_advection_coeff(phi, sgn * mf(index_f), 0, loc_p, loc_nb, face_area, adv_coeff)
          class default
          ...
          !print*,"gamma scheme selection"
            call calc_advection_coeff(phi, mf(index_f), index_nb, loc_p, loc_nb, face_area, adv_coeff)
          class default
            call error_abort("Invalid velocity field discretisation.")
          end select
          aF = adv_coeff
