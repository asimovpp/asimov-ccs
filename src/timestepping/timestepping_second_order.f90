  ...
  module subroutine apply_timestep(mesh, phi, diag, M, b)
    ...
    if (first_update) then
      ...
      do i = 1, local_num_cells
        ...
        ! b = b + V/dt * phi_old
        b_data(i) = b_data(i) + (V / get_timestep()) * phi_old1_data(i)
      end do
      ...
      do i = 1, local_num_cells
        ...
        ! A = A + 1.5*rho*V/dt
        diag_data(i) = diag_data(i) + 1.5 * rho * V / get_timestep()

        ! b = b + rho*V/dt * (2*phi_old(n-1) - 0.5*phi_old(n-2))
        b_data(i) = b_data(i) + rho * V / get_timestep() * (2 * phi_old1_data(i) - 0.5 * phi_old2_data(i))
      end do
      call restore_vector_data(phi%old_values(1)%vec, phi_old1_data)
