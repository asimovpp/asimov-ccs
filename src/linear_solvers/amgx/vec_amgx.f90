!v Submodule file vec_amgx.smod
!
!  An implementation of vector objects using AMGX - the datatype and operations on it
!  (creation, destruction, setting/getting, ...)
!
!  @build amgx
submodule(vec) vec_amgx
#include "ccs_macros.inc"

  use kinds, only: ccs_err
  use amgxtypes, only: vector_amgx
...
contains
        ...
        associate (mesh => vec_properties%mesh)
          ...
          case (cell)
            ...
            associate (nhalo => mesh%topo%halo_num_cells, &
              ...
              call AMGX_vector_create(v%object, resources, mode)
              call AMGX_matrix_create(dummy_matrix%object, resources, mode)

              ! create a dummy matrix to bind parallelisation information
              call AMGX_matrix_upload_all_global(dummy_matrix%object, &
                                                 n_global = mesh%topo%global_num_cells, &
                                                 n = mesh%topo%local_num_cells, &
                                                 nnz = 1, &
                                                 block_dimx = 1, &
                                                 block_dimy = 1, &
                                                 row_pts = dummy_matrix%row_pts, &
                                                 col_indices_global = (/1/), &
                                                 data = (/1.0_ccs_real/), &
                                                 allocated_halo_depth = 1, &
                                                 num_import_rings = 1, &
                                                 partition_vector = mesh%topo%global_indices)

              call AMGX_vector_bind(v%object, dummy_matrix)
              call AMGX_matrix_destroy(dummy_matrix%object)
          ...
          case (face)
            call error_abort("face vector not implemented")
  ...
  module subroutine update_vector(v)
    ...
    type is (vector_amgx)
      ...
      print *, "update_vector not implemented"
