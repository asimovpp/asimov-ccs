...
contains
  ...
  !> Create a new AMGx matrix object.
  module subroutine create_matrix(mat_properties, M)
    ...
    type is (matrix_amgx)
      ...
      type is (parallel_environment_mpi)
        ...
        call AMGX_matrix_create(M%object, resources, mode) 
  ...
  module subroutine finalise_matrix(M)
    ...
    select type (M)
    type is (matrix_amgx)
      call AMGX_matrix_destroy(M%object, ierr)
    end select
  ...
  module subroutine set_matrix_values(mat_values, M)
    ...
    associate (ridx => mat_values%global_row_indices, &
      ...
      type is (matrix_amgx)
        ...
        ! TODO convert global to local indices
        nrows = size(ridx)
        ncols = size(cidx)
        ...
        do id_row = 1, nrows
          if (matmode == add_mode) then
            mode = ADD_VALUES
            M%matrix%add_values(ridx(id_row), val, cidx)
          else if (matmode == insert_mode) then
            mode = INSERT_VALUES
            M%matrix%insert_values(ridx(id_row), val, cidx)
          else
    ...
    type is (matrix_amgx)
      ...
      type is (vector_amgx)
        ...
        type is (vector_amgx)
          ...
          print *, "mat_vec_product not implemented"
