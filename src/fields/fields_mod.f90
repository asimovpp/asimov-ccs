...
#include "ccs_macros.inc"
  ...
  use constants, only: cell_centred_central, cell_centred_upwind, cell_centred_gamma, cell_centred_lupwind, face_centred
  use types, only: field, field_spec, vector_spec, face_field, central_field, upwind_field, gamma_field, lupwind_field
...
contains
  ...
  subroutine allocate_field(vec_properties, field_type, n_boundaries, store_residuals, phi)
    ...
    else if (field_type == cell_centred_lupwind) then
      call dprint("Create linear upwind field")
      allocate (lupwind_field :: phi)
