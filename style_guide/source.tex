...
This document describes the coding style and practices that must be adopted for ASiMoV CCS. Following this recommendations
in this guide will result in consistent and clear code, with good readability and improved maintainability. In addition, 
...
Adhering to the Fortran standard means the code is portable. Using language extensions that are only supported by a subset 
of compilers must be avoided at all cost. All code must follow the Fortran 2018 standard and the file extenion \texttt{.f18} 
should be used by default. In order to verfiy that the standard is being following, the code should be built with 
compile-time checks for standard adherence enabled:
\begin{itemize}
\item Cray Fortran: \texttt{-en}
\item Gnu Fortran: \texttt{-std=f2018}
\item Intel Fortran: \texttt{-std18}
\item AMD Fortran: \texttt{-Mstandard}
\end{itemize}  
...
\begin{itemize}
  ...
  macros, which must be all capitals.
\end{itemize}
...
All dummy argument should be given a declared intent, both to allow for compile time checks and to aid readability.
...
subroutine example(dummy_a, dummy_b)
  integer :: a, b
  b = a
end subroutine
\end{lstlisting}
