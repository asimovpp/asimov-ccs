...
\centerline{\sc Programming practices, style and process guide}
...
\section{Development process guide}
...
An issue tracker should be used to keep track of all developments, such as features and bugs fixes. The following rules must be adhered to:

\begin{itemize}
  \item All features should be given a clear title and description, and they should be associated with a release milestone.
  ...
  be documented.
  \item Issues that are actively being worked on must be moved from the \emph{todo} to the \emph{doing} board.
  \item Issues can be grouped by creating new labels that are assigned to topics that are related or dependent on each other,
   e.g. \texttt{IO} or \texttt{combustion}.
