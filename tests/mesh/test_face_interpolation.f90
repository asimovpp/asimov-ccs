...
contains
  ...
  function generate_mesh(face_coordinate) result(mesh)
    ...
    allocate (mesh%geo%face_normals(ndim, max_faces, total_num_cells))
    mesh%geo%face_normals(:, 1, 1) = (/1.0_ccs_real, 0.0_ccs_real, 0.0_ccs_real/)
    mesh%geo%face_normals(:, 1, 2) = (/-1.0_ccs_real, 0.0_ccs_real, 0.0_ccs_real/)
