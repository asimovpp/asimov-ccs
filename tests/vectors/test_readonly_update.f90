!> @brief Test setting of ghosts cells using readonly vector views
program test_readonly_update
  ...
  real(ccs_real), dimension(:), pointer :: values, values_ro
  ...
  call get_vector_data_readonly(v, values_ro)
  ...
  do i = 1, total_num_cells
    call create_cell_locator(i, loc_p)
    call get_global_index(loc_p, global_index_p)
    if (values_ro(i) /= global_index_p) then
      write (message, *) 'FAIL: wrong vector value. Expected ', global_index_p, ', got ', values_ro(i)
      call stop_test(message)
    end if
  end do

  ! Remove reference to the values array
  call restore_vector_data_readonly(v, values_ro)

  call nullify_mesh_object()

  call fin()

end program test_readonly_update
